...
<%
	List<DesktopPool> list = (List<DesktopPool>)request.getAttribute("list");
...
<html lang="en-us">
		<script type="text/javascript">
			...
			function changeDesc(temp) {
				var option = document.getElementById("option_"+temp.value);
				var array = option.title.split(',');
				document.getElementById("maximumCount").value = array[0];
			//	document.getElementById("minimumCount").value = array[1];
				document.getElementById("allocatedCount").value = array[2];
			}
					...
					<div class="row">
						...
						<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div id="myTable" class="jarviswidget jarviswidget-color-blueDark" data-widget-editbutton="false" data-widget-deletebutton="false">
								...
								-->
								<header>
									<span>&nbsp;&nbsp;<i class="fa fa-edit"></i>&nbsp;&nbsp;<h2>桌面池扩容</h2></span>
								...
								<div>
									...
									<div class="widget-body no-padding">
										<form id="applyForm" class="smart-form" action="${rootUrl }order/doPoolDilatation.do" method="post">
										<input type="hidden" name="unitOu" value="<%=dcode %>"> 
											<fieldset>
												...
												<section class="col col-6">
													...
													<label class="input">
															<input type="text" name="uname" id="uname" style="background:#cccccc" readonly value="<%=uname %>">
													</label>
												</section>
													<section class="col col-6">
														<label class="label">申请部门</label>
														<label class="input">
															<input type="text" name="dname" id="dname" style="background:#cccccc" readonly value="${dname }">
													</label>
												...
												<div class="row">
													<section class="col col-6">
														...
														<label class="input">
														<select class="form-control" name="poolId" id="poolId" style="background:#cccccc" onchange = "changeDesc(this)">
															<%if(list != null && list.size() > 0) {
																for(int i=0;i<list.size();i++) {
																DesktopPool pool = list.get(i);
															%>
																<option id="option_<%=pool.getElmentId() %>" value="<%=pool.getElmentId() %>" title="<%=pool.getMaximumCount()%>,<%=pool.getMinimumCount()%>,<%=pool.getAllocatedCount()%>"><%=pool.getDisplayName()%></option>
															<%
																}
															}%>
														</select>
													...
													<section class="col col-6">
														<label class="label">最大桌面数</label>
														<label class="input">
															<input type="text" name="maximumCount" id="maximumCount" style="background:#cccccc" readonly value="<%if(list != null && list.size()>0){%><%=list.get(0).getMaximumCount()%><%}%>">
														</label>
													</section>
												</div>
												<div class="row">
													<section class="col col-6">
														<label class="label">新增桌面数</label>
														<label class="input">
															<input type="text" name="dilatationCount" id="dilatationCount" >
														</label>
													</section>
													<section class="col col-6">
														<label class="label">已分配桌面数</label>
														<label class="input">
															<input type="text" name="allocatedCount" id="allocatedCount" style="background:#cccccc" readonly value="<%if(list != null && list.size()>0){%><%=list.get(0).getAllocatedCount()%><%}%>">
														</label>
													</section>
												</div>
												<section>
													<label class="label">扩容原因</label>
