...
<html lang="en-us">
									...
									<table id="datatable_tabletools"
										...
										<c:forEach items="${listAccount }" var="account" varStatus="list">
											<tr>
												...
												<td>
													<button class='btn bg-color-red txt-color-white' onclick="deleteAccountFromUnit(${account.id });">删除</button>&nbsp;&nbsp;
												</td>
			...
			"sDom": "<'dt-toolbar'<'col-xs-12 col-sm-6'f><'col-sm-6 col-xs-6 hidden-xs'T>r>"+
      			...
      			"oTableTools": {
      	 			"aButtons": [
						...
						"sButtonText": "返回",
						"fnClick":function(nButton, oConfig, oFlash){
							window.location = "${rootUrl }acl/listOu.do";
						}
					}
	...
	function deleteAccountFromUnit(id){
		$("#isDelete-message").html("<h3>确认删除吗？<h3>")  ;
		$("#isDelete").dialog("open");
		$("#isDelete").dialog({param_id:"${rootUrl}acl/deleteAccountFromUnit.do?id="+id+"&unitId=${unit.id}"});
		 
	}

	function addAccount(){
		//window.location = "${rootUrl}acl/addAccount2Unit.do?unitId=${unit.id}" ;
		var html ='<div id="dialog-window" title="新增人员"><iframe id="myFrame"src="${rootUrl}acl/addAccount2Unit.do?unitId=${unit.id}" frameBorder="0" style="border: 0; " scrolling="no"  class="col-lg-12" height="100%"></iframe></div>';
	    $(html).dialog($.extend({
	      modal: true,
	      width:900,
	      height:600,
