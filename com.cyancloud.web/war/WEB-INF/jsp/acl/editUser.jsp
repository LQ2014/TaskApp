...
<html lang="en-us">
	...
	<body>
		<form id="eidtUserForm"  name="eidtUserForm" class="form-horizontal" action="${rootUrl }acl/doEditUser.do"  method="post" >
			<fieldset>
				...
				<div class="row">
					...
					<div class="col-md-10">
						<input class="form-control required" readonly="readonly" type="text" name="ucode" id="ucode" value="${user.ucode }">
					</div>
				</div>				
				<div class="row">
					<label class="col-md-2 control-label">中文名</label>
					<div class="col-md-10">
						<input class="form-control required" type="text" name="uname" id="uname" value="${user.uname }">
					</div>
				</div>			
				<div class="row">
					...
					<div class="col-md-10">
						<c:forEach items="${accountType }" var="item">
							<c:if test="${user.type==item.key}">
								<input class="required" style="margin-left: 10px;" type="radio" checked="checked" value="${item.key }" name="type" id="type" >${item.value }
							</c:if>
							<c:if test="${user.type!=item.key}">
				...
				<div class="row">
					...
					<div class="col-md-10">
						<input class="form-control required" type="text" name="dateline" id="dateline" value="${user.dateline }">
					</div>
				</div>
				<div class="row">
					<label class="col-md-2 control-label">所属部门</label>
					<div class="col-md-10">
						<input id="unitId" name="unitId" class="mini-buttonedit" allowInput=false onbuttonclick="selectUnit" text="<%if(unitMap!=null){%><%=unitMap.get("value")%><%}%>" value="<%if(unitMap!=null){%><%=unitMap.get("id")%><%}%>"/>   
					</div>
				</div>
			</fieldset>
		</form>
