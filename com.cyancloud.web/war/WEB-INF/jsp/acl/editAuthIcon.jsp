...
<html lang="en-us">
	...
	<body>	
	<% 	
		Right item = (Right)request.getAttribute("right"); 		
	%>
			...
			<form action="" id="edit-icon-form" method="post">
				...
				<input type="hidden" name="icon" id="icon" value="<%=item.getCss()%>">
			</form>
			<!-- end widget div -->	
			<script type="text/javascript">
				$('div.col-xs-6').click(function () {						
					$("div.selected").each(function(){
						$(this).removeClass('selected');
					});
					$(this).addClass('selected');
					var iconCls =$($('div.selected i')[0]).attr('class');
					var dfIcon ="fa fa-lg fa-fw"+ iconCls.substring(2);
					$('#icon').val(dfIcon);	
				});
				$(document).ready(function() {
					var defaultIcon = $('#icon').val();		
					/*$("div .col-xs-6 i").each(function(){	
						var icon = $(this).attr('class');
						icon = icon.substring(2);									
						if(defaultIcon.indexOf(icon)>-1){						
							$(this).parent().addClass('selected');
							return false;
						}				
					});	*/					
					$("div .col-xs-6").css({"cursor":"pointer"});	
					var iconCls = defaultIcon.substring("fa fa-lg fa-fw ".length);
					$("div .col-xs-6 i."+iconCls).parent().addClass('selected');
				});
			</script>			
	</body>
</html>
