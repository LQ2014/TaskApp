...
<html lang="en-us">
	...
	<body id="myBody" onload="onload();">
					...
					<div class="row">
						...
						<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div id="myTable" class="jarviswidget jarviswidget-color-blueDark" data-widget-editbutton="false" data-widget-deletebutton="false">
								...
								-->
								<header>
									<span>&nbsp;&nbsp;<i class="fa fa-table"></i>&nbsp;&nbsp;<h2>员工管理</h2></span>
								...
								<div>
									...
									<div class="widget-body no-padding">
										<table id="datatable_tabletools" class="table table-striped table-bordered table-hover" class="col-lg-12" >
											...
											<tbody>
												<c:forEach items="${employeeList }" var="vo" varStatus="list">
												<tr>
													<td>${list.count}</td>
													<td>${vo.code }</td>
													<td>${vo.name }</td>
													...
													<td>${vo.deptName}</td>
													<td>
														<c:if test="${vo.isShowScore==0}">${vo.score}</c:if>
													</td>
													<td>
														<c:if test="${vo.isShowScore==0}">
															<a href="${rootUrl }acl/isShowScore.do?id=${vo.id}" class="btn bg-color-green txt-color-white">隐藏积分</a>
														</c:if>
														<c:if test="${vo.isShowScore==2}">
															<a href="${rootUrl }acl/isShowScore.do?id=${vo.id}" class="btn bg-color-yellow txt-color-white">显示积分</a>
														</c:if>
														<button class='btn bg-color-blue txt-color-white' onclick="editEmployee(${vo.id });">修改</button>&nbsp;&nbsp;
														<button class='btn bg-color-red txt-color-white' onclick="deleteEmployee(${vo.id });">删除</button>&nbsp;&nbsp;
													</td>
												</tr>	
												</c:forEach>
		...
		function addEmployee(){
			var html ='<div id="dialog-window" title="新增员工"><iframe id="myFrame"src="${rootUrl}acl/addEmployee.do" frameBorder="0" style="border: 0; " scrolling="auto"  class="col-lg-12" height="100%"></iframe></div>';
		    $(html).dialog($.extend({
		      modal: true,
		      width:650,
		      height:500,
		      ...
		      buttons : [{
				...
				click : function(btn) {
					...
					var myForm = myFrameWin.document.getElementById("addEmployeeForm");
					ajaxSubmit(myForm);
				}
			}]			      
		...
		function editEmployee(id){
			var html ='<div id="dialog-window" title="修改员工"><iframe id="myFrame" src="${rootUrl}acl/editEmployee.do?id='+id+'" frameBorder="0" style="border: 0;" scrolling="auto" class="col-lg-12" height="100%"></iframe></div>';
		    return $(html).dialog($.extend({
		      ...
		      buttons : [{
				...
				click : function(btn) {
					var myFrameWin = document.getElementById("myFrame").contentWindow;
					var myForm = myFrameWin.document.getElementById("eidtEmployeeForm");
					ajaxSubmit(myForm);
				}
			}]
		...
		function check(frm){			
			var els = $(frm).find(" .required");
			var isSuccess = true;
			if (els.length > 0) {
				for (var i = 0; i < els.length; ++i) {
					els.eq(i).unbind("focus");
					if ('' == els.eq(i).val() || els.eq(i).val() == els.eq(i).attr("message")) {
						els.eq(i).val(els.eq(i).attr("message"));
						els.eq(i).css({"color":"red"});
						if (els.eq(i).attr("type") == "password") {
							els.eq(i).attr("type", "text");
						}
						isSuccess = false;
						els.eq(i).unbind("focus");
						els.eq(i).bind("focus", function(src){
							$(src.currentTarget).val("");
							$(src.currentTarget).css({"color":""});
							if ($(src.currentTarget).attr("stype") == "password") {
								$(src.currentTarget).attr("type", "password");
							}
						});
					}
				}
		...
		function deleteEmployee(id){
			$("#isDelete-message").html("<h5>确认删除员工信息？<h5>")  ;
			$("#isDelete").dialog("open");
			$("#isDelete").dialog({param_id:"${rootUrl}acl/deleteEmployee.do?id="+id});
	 	}
