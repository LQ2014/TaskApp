...
<html lang="en-us">
		<script type="text/javascript" language="javascript">
			function approve(machineId,state) {
     			ajaxFunction('${rootUrl }order/operateApprove.do?machineId='+machineId+'&state='+state);
    		}
		</script>
					...
					<div class="row">
						...
						<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div id="myTable" class="jarviswidget jarviswidget-color-blueDark" data-widget-editbutton="false" data-widget-deletebutton="false">
								...
								-->
								<header>
									<span>&nbsp;&nbsp;<i class="fa fa-table"></i>&nbsp;&nbsp;<h2>待审批列表</h2></span>
										...
										<table id="datatable_col_reorder" class="table table-striped table-bordered table-hover">
											<thead>
												<tr>
													...
													<th data-class="expand">申请人</th>
													<th>申请时间</th>
													<th data-hide="phone">CPU</th>
													<th data-hide="phone,tablet">内存(GB)</th>
													<th data-hide="phone,tablet">硬盘(GB)</th>
													<th data-hide="phone,tablet">预装系统</th>
													<th data-hide="phone,tablet">系统类型</th>
													<th data-hide="phone,tablet">使用期限</th>
													<th data-hide="phone,tablet">申请用途</th>
													<th data-hide="phone,tablet">审批</th>
												</tr>
											</thead>
											<tbody>
												<%if(machineList != null && machineList.size() > 0) {
													for(int i=0;i<machineList.size();i++) {
														...
														Calendar c = machine.getCtime();
														SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
														String life = sdf.format(c.getTime());
														Long machineId = machine.getId();
												%>
													<tr>
													<td><%=i+1%></td>
													<td><%=machine.getUserId()%></td>
													<td><%=life%></td>
													<td><%=machine.getCpu()%></td>
													<td><%=machine.getMemory()%></td>
													<td><%=machine.getHdisk()%></td>
													<td><%=machine.getOs()%></td>
													<td><%=machine.getVersion()%></td>
													<td><%=machine.getLife()%></td>
													<td><%=machine.getReason()%></td>
													<td>
														<input type="button" value="通过" onclick="approve('<%=machineId%>','8');"/>&nbsp;
														<input type="button" value="退回" onclick="approve('<%=machineId%>','-1');"/>&nbsp;
													</td>
												</tr>	
												<%
													}
