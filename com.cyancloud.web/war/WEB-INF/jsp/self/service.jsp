...
<% 
String path="self/service";
String uname=(String)session.getAttribute("ucode");
List<SystemParam> params = (List<SystemParam>)request.getAttribute("params");
List<ServiceItem> items = (List<ServiceItem>)request.getAttribute("items");
%>
				...
				<div class="row">
					<!-- NEW WIDGET START -->
						...
						<%
							...
							SystemParam param = params.get(i);
						%>
						<!-- Widget ID (each widget will need unique ID)-->
							<div class="jarviswidget jarviswidget-color-blueDark" id="myTable<%=i+1%>" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false">
								<!-- widget options:
								...
								<div>
									...
									<div class="widget-body">
										...
										<%		for(int j=0;j<items.size();j++){
											ServiceItem item = items.get(j);
											if(item.getType().equals(String.valueOf(param.getId()))){
										%>
										<li>
												<a href="${rootUrl }<%=item.getUrl()%>" title="<%=item.getDescription() %>"><span class="<%=item.getIcon() %>"></span>
												<span class="glyphicon-class"><%=item.getName() %></span></a>
