...
<html lang="en-us">
	...
	<body id="myBody" onload="onload();">
					...
					<div class="row">
						...
						<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
							...
							<div id="myTable" class="jarviswidget jarviswidget-color-blueDark" data-widget-editbutton="false" data-widget-deletebutton="false">
								...
								-->
								<header>
									<span>&nbsp;&nbsp;<i class="fa fa-table"></i>&nbsp;&nbsp;<h2>参数配置</h2></span>
								...
								<div>
									...
									<div class="widget-body no-padding">
										<table id="datatable_tabletools" class="table table-striped table-bordered table-hover" class="col-lg-12" >
											...
											<tbody>
												<c:forEach items="${configList }" var="vo" varStatus="list">
												<tr>
													<td>${list.count}</td>
													<td>${vo.key }</td>
													<td>${vo.value }</td>
													...
													<td>${vo.note}</td>
													<td>
														<button class='btn bg-color-blue txt-color-white' onclick="editConfig('${vo.id }');">修改</button>&nbsp;&nbsp;
														<c:if test="${vo.isEdit==1}">
															<button class='btn bg-color-red txt-color-white' onclick="deleteConfig('${vo.id }');">删除</button>&nbsp;&nbsp;
														</c:if>
													</td>
												</tr>	
												</c:forEach>
		...
		function addConfig(){
			var html ='<div id="dialog-window" title="新增参数"><iframe id="myFrame"src="${rootUrl}acl/addConfig.do" frameBorder="0" style="border: 0; " scrolling="no"  class="col-lg-12" height="100%"></iframe></div>';
		    $(html).dialog($.extend({
		      modal: true,
		      width:620,
		      height:450,
		      ...
		      buttons : [{
				...
				click : function(btn) {
					...
					var myForm = myFrameWin.document.getElementById("addConfigForm");
					ajaxSubmit(myForm);
				}
			}]			      
		...
		function editConfig(id){
			var html ='<div id="dialog-window" title="修改参数"><iframe id="myFrame" src="${rootUrl}acl/editConfig.do?id='+id+'" frameBorder="0" style="border: 0;" scrolling="no" class="col-lg-12" height="100%"></iframe></div>';
		    return $(html).dialog($.extend({
		      ...
		      buttons : [{
				...
				click : function(btn) {
					var myFrameWin = document.getElementById("myFrame").contentWindow;
					var myForm = myFrameWin.document.getElementById("eidtConfigForm");
					ajaxSubmit(myForm);
				}
			}]
		...
		function deleteConfig(id){
			$("#isDelete-message").html("<h5>确认删除参数信息？<h5>")  ;
			$("#isDelete").dialog("open");
			$("#isDelete").dialog({param_id:"${rootUrl}acl/deleteConfig.do?id="+id});
	 	}
